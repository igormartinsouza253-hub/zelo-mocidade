...
const validarReferenciabiblica = (referencia: string) => {
  // Formato esperado: "Livro Capítulo:Versículo" (ex: "Salmos 23:1")
  const regex = /^(.+?)\s+(\d+):(\d+)$/;
  ...
  if (!bibliaLivros[livro]) return false;
  
  // Verifica se o capítulo é válido
  if (capituloNum < 1 || capituloNum > bibliaLivros[livro]) return false;
  
  // Versículo deve ser pelo menos 1
  if (versiculoNum < 1) return false;
...
const reuniaoSchema = z.object({
  data: z
    ...
    .refine((date) => {
      ...
      return meetingDate >= minDate && meetingDate <= maxDate;
    }, "Data deve estar entre 2000 e um ano no futuro"),
  tema: z
    .string()
    .trim()
  ...
  quem_atendeu: z
    .string()
    .trim()
    .max(200, "Quem Atendeu deve ter no máximo 200 caracteres")
    .optional(),
  palavra_referencia: z
    .string()
    .trim()
    .refine(
      (val) => !val || validarReferenciabiblica(val),
      "Referência bíblica inválida. Use o formato: Livro Capítulo:Versículo (ex: Salmos 23:1)"
    )
    .optional(),
});
...
const NovaReuniao = () => {
  ...
  const toggleCargo = (cargoNome: string) => {
    setSelectedCargos((prev) =>
      prev.includes(cargoNome)
        ? prev.filter((c) => c !== cargoNome)
        : [...prev, cargoNome]
    );
  };
  ...
  const filteredMembros = membros.filter((membro) => {
    ...
    const matchesCargo =
      ...
      selectedCargos.some((cargo) => cargosDoMembro.includes(cargo));
    const matchesFaixa =
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validation = reuniaoSchema.safeParse({
      data: formData.data,
      tema: formData.tema || undefined,
      observacoes: formData.observacoes || undefined,
      quem_atendeu: formData.quem_atendeu || undefined,
      palavra_referencia: formData.palavra_referencia || undefined,
    });
    ...
    if (
      ...
      (!formData.numero_visitas || formData.numero_visitas === 0)
    ) {
      toast.error("Registre pelo menos um membro presente ou uma visita.");
      return;
    }
        ...
        .insert([
          {
            ...
            // Mantemos o campo antigo de orações como nulo (agora o registro é por membro na lista)
            oracoes: null,
          },
        ])
        .select()
    ...
    <div className="w-full h-full flex justify-start">
      ...
      >
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
              ...
              <Card className="h-full rounded-3xl shadow-[var(--shadow-soft)] border-border/50">
                <CardHeader>
                  ...
                  {!isMobile && (
                    <p className="text-sm text-muted-foreground">
                      ...
                      {formData.recitativos_individuais > 0 && (
                        <span className="ml-2 text-primary font-semibold">
                          | Total Recitativos: {" "}
                          {selectedMembros.length +
                            formData.numero_visitas +
                            formData.recitativos_individuais}
                        </span>
                      )}
                    </p>
                  )}
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-3 max-h-[400px] overflow-y-auto scrollbar-none">
                    ...
                    {!membrosLoading && filteredMembros.length === 0 && (
          ...
          {/* Ações movidas para a barra superior */}
        </form>
      </div>
