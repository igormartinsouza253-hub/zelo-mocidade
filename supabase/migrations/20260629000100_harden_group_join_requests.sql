-- Make group join requests reliable for brand-new authenticated accounts.
-- The request flow must not depend on the legacy global user_roles bootstrap.

CREATE UNIQUE INDEX IF NOT EXISTS group_join_requests_group_user_key
...
DROP POLICY IF EXISTS "Authenticated users can list groups" ON public.management_groups;
...
GRANT SELECT ON public.management_groups_public TO authenticated;
GRANT EXECUTE ON FUNCTION public.request_group_join(uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.check_group_password(uuid, text) TO authenticated;
